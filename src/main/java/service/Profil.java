...
public class Profil extends HttpServlet {
		...
		try{
			 ...
			 if (session.isNew()){
			        ...
			        if(u.getMon_contrat() == null){
			    		...
			    		ecrire.write("<style>td {vertical-align:top; padding: 100px; padding-top: 100px; width:300px; text-align:left;} .td1{vertical-align:top; }</style>");
			    	...
			    	//	ecrire.write("<div align='left' style=' position:relative;top:20px; left:60px; width:50px;'>");
			    		ecrire.write("<div class='alert alert-success' role='alert' width='300' ><strong>Donnees personels</strong></div>");
                        ecrire.write(" <h4>Prenom: </h4>"+u.getFirstName());
                        ecrire.write(" <h4>Nom: </h4>"+u.getName());
                        ecrire.write(" <h4>Date de naissance: </h4>"+u.getDateNaissance());
                        String s;
                        if(u.getSexe() == 0)
                        	s="Homme";
                        else s="Femme";
                        ecrire.write(" <h4>Sexe: </h4>"+s);
                        ecrire.write(" <h4>E-mail: </h4>"+u.getEmail());
                        ecrire.write(" <h4>Contrat: </h4>"+u.getMon_contrat().getCommercial_name());
			    		ecrire.write("</div></td>"); 
			    		ecrire.write("<td><div class='alert alert-warning' role='alert'><strong>Veillez saisir vos nouvelles donnees:</strong></div>");		 
			        	ecrire.write("<form method='post' action='Profil'><br><i>Email: <input type='text' name='email'></i><br><i>Password: <input type='password' name='password'></i><br>Confirmer Password: <input type='password' name='password2' size='25'><br>Nom: <input type='text' name='name' size='25'><br>Prenom: <input type='text' name='firstname' size='25'><br>Sexe: <label><input type='radio' name='sexe' value='femme'>femme</label><label><input type='radio' name='sexe' value='homme'>homme</label><br>Date de naissance  <input type='text' name='datenaissance' size='25'> <br>sans separateurs, ex: 18011986<br><input class='button' type='submit' name='inscription' value='Modifier' /> <input class='button' type='reset' value='Reset'></form></div>");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// on rentre ici si l'utilisateur est connecte
				...
				try{
					 ...
					 if (session.isNew()){
							response.sendRedirect("index.html"); 
				      } else {// connecte, possibilite de tout modifier, c'est dangereux niveau securite ?
					        ...
					        if(u.getMon_contrat() == null){
					        	...
					        	if(insc != null){  // inscription
									...
									if(sexe == null || !(password.equals(password2)) || email.equals("") || password.equals("") || password2.equals("") || name.equals("") || firstname.equals("") || datenaissance.equals("")){
										...
										// possibilite de tout modifier, c'est dangereux niveau securite ?
										...
										// il faut afficher le profil et les changements effectues
											response.sendRedirect("Profil");
											}
					        	}
					        }
